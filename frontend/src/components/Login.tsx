...
const Login: React.FC = () => {
    ...
    const handleStaffVerify = async (e: React.FormEvent) => {
        ...
        try {
            user = await verifyLoginOtp(email, password, otp);
        ...
        <>
            {step === 1 ? (
                ...
                <form onSubmit={handleStaffVerify}>
                    ...
                    <div className="space-y-4">
                         <Input label="Security Code" id="otp" type="text" value={otp} onChange={(e) => setOtp(e.target.value)} required placeholder="Enter 6-digit code"/>
                    </div>
                    <Button type="submit" className="w-full mt-6 bg-rose-600 hover:bg-rose-700" disabled={isLoading}>
             ...
             <>
                {step === 1 ? (
                    ...
                    <form onSubmit={handleStaffVerify}>
                         ...
                         <p className="text-center text-slate-500 dark:text-slate-400 mb-6">A real verification code has been sent to your email. <br/></p>
                        <div className="space-y-4">
