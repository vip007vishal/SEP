...
import { findUserByEmail, createTeacherUser } from './examService';
...
export const registerTeacher = (name: string, email: string, password: string): Promise<User | null> => {
    // Delegate teacher creation to the centralized examService
    return createTeacherUser(name, email, password);
};
