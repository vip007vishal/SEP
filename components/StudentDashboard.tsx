...
const StudentDashboard: React.FC = () => {
    ...
    useEffect(() => {
        const fetchStudentExams = async () => {
            ...
            setIsLoading(true);
            const fetchedExams = await getExamsForStudent(user.registerNumber);
            ...
            <div className="min-h-screen bg-slate-50">
                ...
                <main className="max-w-screen-2xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                    ...
                    <Card className="mb-8">
                        ...
                        <p className="text-slate-500 mb-4">Date: {selectedExam.date}</p>
                        
                        {studentSeat && hall ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-center p-4 bg-violet-50 rounded-lg">
                                <div>
                                    <p className="text-sm text-slate-500">Your Hall</p>
                             ...
                             <div className="text-center p-4 bg-slate-100 rounded-lg">
                                <p className="text-slate-600">Your seat details could not be found for this exam.</p>
                            </div>
                        )}
                    </Card>

                    <h3 className="text-2xl font-bold text-slate-800 mb-6 text-center">Seating Arrangements</h3>
    ...
    return (
        <div className="min-h-screen bg-slate-50">
            ...
            <main className="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                <h2 className="text-3xl font-bold text-slate-800 mb-6">My Assigned Exams</h2>
                {assignedExams.length > 0 ? (
                    <div className="space-y-4">
                        {assignedExams.map(exam => (
                            <Card 
                                key={exam.id}
                                className="hover:shadow-lg hover:border-violet-300 border border-transparent transition-all cursor-pointer"
                                onClick={() => setSelectedExam(exam)}
                            >
                                <div className="flex items-center justify-between">
                                    <div>
                                        <p className="font-semibold text-lg text-violet-700">{exam.title}</p>
                    ...
                    <Card>
                        <div className="text-center py-10">
                            <p className="text-slate-500">You have no upcoming exams scheduled at this time.</p>
                        </div>
                    </Card>
                )}
            </main>
