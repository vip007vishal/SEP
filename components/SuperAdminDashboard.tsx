...
const SuperAdminDashboard: React.FC = () => {
    ...
    const handleGrantAccess = async (adminId: string) => {
        if (window.confirm("Grant administrator access to this institution?")) {
            await grantAdminPermission(adminId);
            await fetchData();
        }
    };
                ...
                {activeTab === 'institutes' && (
                    <div className="space-y-8">
                                    ...
                                    {pendingAdmins.map(admin => (
                                        <Card key={admin.id} className="!p-5 border-amber-200 shadow-sm">
                                            ...
                                            <div className="flex gap-2">
                                                <Button size="sm" className="flex-1 !bg-emerald-600 !hover:bg-emerald-700" onClick={() => handleGrantAccess(admin.id)}>
                                                    Approve Access
                                                </Button>
                                                <button 
                        ...
                        <Card>
                            ...
                            {isLoading ? <p className="text-center py-12 text-slate-500">Loading infrastructure data...</p> : (
                                        ...
                                        <div key={admin.id} className="bg-white dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 flex flex-col hover:border-violet-300 dark:hover:border-violet-900 transition-colors">
                                            ...
                                            <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center">
                                                {!admin.permissionGranted ? (
                                                    <Button size="sm" onClick={() => handleGrantAccess(admin.id)}>Approve</Button>
                                                ) : (
                                                ...
                                                <button 
                                                    onClick={() => handleDeleteInstitution(admin.id, admin.institutionName!)} 
                                                    className="flex items-center gap-2 text-xs font-black text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 px-3 py-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
                                                >
